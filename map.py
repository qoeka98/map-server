...
- **챗봇의 템퍼러쳐(temperature)는 1.3로 설정하여** 상황에 따른 적절한 답변을 제공하도록 했습니다. 템퍼러쳐가 0이면 자연재해에 대한 답변을 제공하지만 특정 유연성이 필요한 질문은 대답을 잘 하지 못했으며, 2는 답변의 정확성을 떨어뜨리므로 1.3로 설정하여 **상황에 맞는 답변을 제공**할 수 있도록 조정했습니다.
