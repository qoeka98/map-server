...
def run_sangdam():
    ...
    # ✅ 기존 채팅 기록을 최신 순으로 위에 표시
    st.markdown(f"### 📂 저장된 대화 기록 (최신 질문과 답변이 위에 표시됩니다)")
    for i, msg in enumerate(reversed(st.session_state.messages), start=1):
        if msg["role"] == "user":
    ...
    chat = st.text_input("지진 관련 질문을 입력하세요!", key="chat_input")
    ...
    if chat:
        ...
        if not is_earthquake_related(clean_chat):
            ...
            특히 쓰나미 대처법이나 안전 대책에 대해서도 구체적으로 안내해.
            '''
